...
function AdminDashboard() {
  ...
  const maxChartValue = useMemo(() => Math.max(...(data?.dailySales || []).map((item) => Number(chartMetric === "orders" ? item.orders : item.revenue) || 0), 1), [chartMetric, data]);
  const hasChartData = (data?.dailySales || []).some((item) => Number(chartMetric === "orders" ? item.orders : item.revenue) > 0);
  const cards = [
  ...
  return <SellerPage>
    ...
    {!loading && !data ? <SellerEmpty title="Analytics are temporarily unavailable" description="Refresh the page while the store reconnects to reporting data."/> : <>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,.5fr)]">
        <section className="rounded-2xl border bg-white p-5 shadow-sm md:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">Daily sales activity</h2><p className="mt-1 text-sm text-slate-500">Compare placed orders with revenue realized after delivery.</p></div><div className="flex rounded-xl bg-slate-100 p-1"><button type="button" onClick={() => setChartMetric("revenue")} className={`rounded-lg px-3 py-2 text-xs font-semibold ${chartMetric === "revenue" ? "bg-white text-brand-primary shadow-sm" : "text-slate-500"}`}>Revenue</button><button type="button" onClick={() => setChartMetric("orders")} className={`rounded-lg px-3 py-2 text-xs font-semibold ${chartMetric === "orders" ? "bg-white text-brand-primary shadow-sm" : "text-slate-500"}`}>Orders</button></div></div>{!hasChartData && !loading && <div className="mt-6 rounded-xl border border-dashed border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">{chartMetric === "revenue" ? "No delivered orders in this period yet. Switch to Orders to see placed-order activity." : "No orders were placed in this period."}</div>}<div className="mt-8 flex h-56 items-end gap-1.5 border-b border-slate-200 px-1" aria-label={`Daily ${chartMetric} chart`}>{(data?.dailySales || []).map((item, index) => { const value = Number(chartMetric === "orders" ? item.orders : item.revenue) || 0; return <div key={item.date} className="group relative flex h-full min-w-0 flex-1 items-end"><div style={{ height: `${Math.max((value / maxChartValue) * 100, value ? 5 : 1)}%` }} className={`w-full rounded-t transition ${chartMetric === "orders" ? "bg-[#a77b45] hover:bg-[#805b2f]" : "bg-[#397153] hover:bg-[#1f4b34]"}`}/><span className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded-lg bg-slate-950 px-2 py-1 text-xs text-white group-hover:block">{new Date(item.date).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}: {chartMetric === "orders" ? `${item.orders} placed` : `${currency(item.revenue)} · ${item.deliveredOrders || 0} delivered`}</span>{(data.dailySales.length <= 14 || index % Math.ceil(data.dailySales.length / 8) === 0) && <span className="absolute top-full mt-2 text-[10px] text-slate-400">{new Date(item.date).getDate()}</span>}</div>; })}</div></section>
