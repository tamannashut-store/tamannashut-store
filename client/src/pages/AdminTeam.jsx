...
function AdminTeam() {
  ...
  return <SellerPage><SellerHeader title="Seller team" description="Invite authorised sellers and review business, GST and settlement details." />
    ...
    <section className="mt-6 overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="border-b px-5 py-5 md:px-6"><h2 className="font-semibold">Independent seller accounts</h2><p className="mt-1 text-sm text-slate-500">Each verified seller receives an isolated catalogue, order view and business profile. They never receive platform-administrator access.</p></div>{!loading && !data.sellers?.length ? <SellerEmpty title="No seller accounts" description="Send the first secure invitation above."/> : <div className="divide-y">{data.sellers?.map((seller) => <article key={seller._id} className="p-5 md:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{seller.name}</h3><StatusBadge value={seller.profile?.verificationStatus || "Pending"}/><StatusBadge value={seller.sellerAccessStatus}/></div><p className="mt-1 text-sm text-slate-500">{seller.email} · Independent marketplace seller</p></div>{seller.profile && ["pending", "needs_update"].includes(seller.profile.verificationStatus) && <div className="flex gap-2"><button onClick={() => review(seller, "rejected")} className="inline-flex items-center gap-2 rounded-xl border border-red-200 px-4 py-2 text-sm font-semibold text-red-700"><FiX/> Request correction</button><button disabled={!seller.profile.completeness?.complete} onClick={() => review(seller, "verified")} className="btn-primary disabled:cursor-not-allowed disabled:opacity-40"><FiCheck/> Verify</button></div>}</div>
          {seller.profile ? <div className="mt-5 space-y-4">
            {!seller.profile.completeness?.complete && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><strong>Approval blocked — incomplete application.</strong><span className="mt-1 block">Missing: {seller.profile.completeness?.missingFields?.join(", ") || "required seller details"}. The seller can correct business and fulfilment fields from Business profile.</span></div>}
