...
function EditProduct() {
  ...
  const addImages = (event) => {
    ...
    const next = files.map((file, index) => { const url = URL.createObjectURL(file); previewUrls.current.push(url); return { id: `new-${Date.now()}-${index}`, type: "new", file, url, color: form.color || "", size: "" }; });
    ...
    <div className="p-5 md:p-8 xl:p-10">
      ...
      <form onSubmit={submit} className="mt-8">
        ...
        {editStep >= 2 && <div className="mx-auto max-w-5xl space-y-6">
          <section className="surface-card p-6"><h3 className="font-semibold">Colour and size assignment</h3><p className="mt-1 text-sm text-slate-500">Assign a photo to an exact variation, or leave size blank when it applies to every size in that colour.</p><div className="mt-4 space-y-3">{images.map((image, index) => <div key={`assignment-${image.id}`} className="grid items-center gap-3 rounded-xl border p-3 sm:grid-cols-[56px_1fr]"><img src={image.url} alt="" className="h-14 w-14 rounded-lg object-cover"/><ImageVariantAssignment color={image.color || ""} size={image.size || ""} colors={variantColors} variants={variants} onChange={(assignment) => setImages((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, ...assignment } : item))} /></div>)}</div></section>
