...
import { FiChevronRight, FiHeart, FiLogOut, FiMenu, FiMessageSquare, FiPackage, FiSearch, FiShoppingBag, FiUser, FiX } from "react-icons/fi";
...
function Navbar() {
  ...
  const accountType = userData?.user?.accountType || (userData?.user?.sellerRole === "member" ? "seller" : userData?.user?.isAdmin ? "platform_admin" : "customer");
  const sellerCentrePath = accountType === "seller" ? "/seller/dashboard" : accountType === "platform_admin" ? "/admin/dashboard" : "";
  ...
  return <>
    ...
    <header className="sticky top-0 z-50 border-b border-[#e8e5de] bg-[#fcfbf8]/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[82px] max-w-[1400px] items-center gap-5 px-4 sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          ...
          {userData ? <div className="relative hidden md:block"><button type="button" onClick={() => setProfileOpen((open) => !open)} aria-label="Account" className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 text-[#183d2b]"><FiUser size={19}/></button>{profileOpen && <div className="absolute right-0 mt-3 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white py-2 shadow-xl">{accountType === "customer" && <><Link to="/profile" className="block px-4 py-2.5 text-sm hover:bg-slate-50">Profile</Link><Link to="/my-orders" className="block px-4 py-2.5 text-sm hover:bg-slate-50">My orders</Link><Link to="/support" className="block px-4 py-2.5 text-sm hover:bg-slate-50">Support requests</Link></>}{sellerCentrePath && <Link to={sellerCentrePath} className="block px-4 py-2.5 text-sm hover:bg-slate-50">Seller Centre</Link>}<button type="button" onClick={logout} className="block w-full border-t border-slate-100 px-4 py-2.5 text-left text-sm text-red-600 hover:bg-red-50">Sign out</button></div>}</div> : <div className="hidden items-center gap-2 md:flex"><Link to="/login" className="rounded-full border border-[#183d2b] px-4 py-2 text-sm font-semibold text-[#183d2b] hover:bg-[#eef3ee]">Sign in</Link><Link to="/register" className="rounded-full bg-[#183d2b] px-4 py-2 text-sm font-semibold text-white hover:bg-[#24583f]">Register</Link></div>}
    ...
    {menuOpen && <div className="fixed inset-0 z-[70] lg:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-[min(88vw,360px)] flex-col overflow-y-auto bg-[#fcfbf8] shadow-2xl">
        ...
        <nav className="space-y-1 px-3" aria-label="Account navigation"><Link to="/wishlist" onClick={() => setMenuOpen(false)} className="flex min-h-13 items-center gap-3 rounded-xl px-3 font-medium text-slate-700 hover:bg-[#eef3ee]"><FiHeart/> Wishlist</Link><Link to="/cart" onClick={() => setMenuOpen(false)} className="flex min-h-13 items-center gap-3 rounded-xl px-3 font-medium text-slate-700 hover:bg-[#eef3ee]"><FiShoppingBag/> Cart {cartCount > 0 && <span className="ml-auto rounded-full bg-[#b94545] px-2 py-0.5 text-xs font-bold text-white">{cartCount}</span>}</Link>{userData && accountType === "customer" && <><Link to="/my-orders" onClick={() => setMenuOpen(false)} className="flex min-h-13 items-center gap-3 rounded-xl px-3 font-medium text-slate-700 hover:bg-[#eef3ee]"><FiPackage/> My orders</Link><Link to="/support" onClick={() => setMenuOpen(false)} className="flex min-h-13 items-center gap-3 rounded-xl px-3 font-medium text-slate-700 hover:bg-[#eef3ee]"><FiMessageSquare/> Support requests</Link></>}</nav>
        <div className="mt-auto border-t border-slate-200 p-5">{userData ? <div className="space-y-2">{accountType === "customer" && <Link to="/profile" onClick={() => setMenuOpen(false)} className="btn-primary w-full"><FiUser className="mr-2"/> My account</Link>}{sellerCentrePath && <Link to={sellerCentrePath} onClick={() => setMenuOpen(false)} className="btn-primary w-full">Seller Centre</Link>}<button type="button" onClick={logout} className="flex w-full items-center justify-center gap-2 py-2 text-sm font-semibold text-red-600"><FiLogOut/> Sign out</button></div> : <div className="grid gap-2"><Link to="/login" onClick={() => setMenuOpen(false)} className="btn-secondary w-full"><FiUser className="mr-2"/> Sign in</Link><Link to="/register" onClick={() => setMenuOpen(false)} className="btn-primary w-full">Create account</Link></div>}</div>
      </aside>
    </div>}
  </>;
}
